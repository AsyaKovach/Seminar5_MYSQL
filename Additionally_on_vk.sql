...
-- Task1 Получите друзей пользователя с id=1
CREATE OR REPLACE VIEW users_friends AS 
SELECT *
FROM users AS u
...
-- Task2 Создайте представление, в котором будут выводится все сообщения, в которых принимал участие пользователь с id = 1.
...
-- Task3 Получите список медиафайлов пользователя с количеством лайков(media m, likes l ,users u)
CREATE OR REPLACE VIEW users_media_likes AS 
SELECT m.id, m.filename AS "Медиафайлы", count(l.id) AS "Количество лайков"
FROM media m
LEFT JOIN likes AS l ON l.media_id = m.id
JOIN users AS u ON u.id = m.user_id
GROUP BY m.id
ORDER BY count(l.id) DESC;
SELECT * FROM users_media_likes;

-- Task4 Получите количество групп у пользователей
CREATE OR REPLACE VIEW users_communities_total AS 
SELECT CONCAT(u.firstname, ' ', u.lastname) AS "Пользователь", count(*) AS "Всего групп"
FROM users AS u
JOIN users_communities AS uc ON u.id = uc.user_id
GROUP BY u.id
ORDER BY count(*) DESC;
SELECT * FROM users_communities_total;

-- Task5_1 Создайте представление, в которое попадет информация о пользователях (имя, фамилия, город и пол), которые не старше 20 лет.
CREATE OR REPLACE VIEW view_user AS 
SELECT firstname AS "Пользователи (младше 20 лет)", lastname AS "Фамилия", hometown AS "Город", gender AS "Пол" 
FROM users u
...
/* Task6_2 Найдите кол-во, отправленных сообщений каждым пользователем и выведите ранжированный список пользователей, указав имя и фамилию пользователя, 
...
CREATE OR REPLACE VIEW users_messages_reiting AS    
SELECT 
...
GROUP BY u.id;
SELECT * FROM users_messages_reiting;

/*Task7_3 Выберите все сообщения, отсортируйте сообщения по возрастанию даты отправления (created_at)
 и найдите разницу дат отправления между соседними сообщениями, получившегося списка. (используйте LEAD или LAG)
 */
CREATE OR REPLACE VIEW messages_total_lead AS    
SELECT *, 
...
SELECT * FROM messages_total_lead;
